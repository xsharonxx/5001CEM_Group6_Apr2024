                ...
                for t in time_list:
                    ...
                    if start_work <= datetime_t <= end_work:
                        ...
                        if datetime_t not in booked_time_list:
                            ...
                            time_button_list[t][0].state(['!disabled'])
            elif (working and len(working) < 1) or (len(operation) > 1 and operation[1] != '24 hours'):
    ...
    def set_up_timetable_frame(self):
        ...
        def view_appointments():
            ...
            def parse_doctor_workinghours(workinghours):
                time_format_12 = '%I%p'
                # Parse the working hours to get rest days
            ...
            time_slots = [time(hour, 0, 0) for hour in range(8, 20)]  # 8 AM to 8 PM
            ...
            elif (working_list and len(working_list) < 1) or (len(c_operation) > 1 and c_operation[1] != '24 hours'):
                time_format_12 = '%I%p'
        ...
        columns = ("time", "status")
        tree = ttk.Treeview(date_doctor_frame, columns=columns, show='headings', height=12)
...
class Doctor:
    ...
    def set_up_timetable_frame(self):
        ...
        view_button.place(x=850, y=40)

        # Date selection dropdown
        date_entry_frame = tk.Frame(date_doctor_frame, bg='#D0F9EF', width=200, height=45)
        date_entry_frame.place(x=170, y=40)
        date_entry = tk.Label(date_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585')
        date_entry.place(x=10, y=10)
        date_entry.config(text='Select Date')

        def show_calendar():
            calendar_frame.place(x=170, y=85)
            calendar_frame.lift()
            if self.tree:
                self.tree.place_forget()
            if self.doctor_name_label:
                self.doctor_name_label.place_forget()
        ...
        def select_date():
            selected_date = cal.get_date()
            date_entry.config(text=selected_date)
            hide_calendar()

        calendar_button = ttk.Button(date_entry_frame, style='calendar.TButton', cursor='hand2', command=show_calendar)
        calendar_button.place(x=150, y=5)
        ...
        calendar_buttons_frame = tk.Frame(calendar_frame, bg='#D0F9EF')
        calendar_buttons_frame.pack()
        ttk.Button(calendar_buttons_frame, text="Select", command=select_date).pack(side='left', padx=5)
        ttk.Button(calendar_buttons_frame, text="Cancel", command=hide_calendar).pack(side='left', padx=5)
        ttk.Button(calendar_buttons_frame, text="Remove", command=remove_date).pack(side='left', padx=5)
        calendar_frame.place_forget()  # Initially hide the calendar frame
        ...
        def doctor_view_appointments():
            ...
            if selected_date_obj < current_date:
                messagebox.showerror("Error", "The selected date is in the past. Please choose a valid date.")
                return

            def fetch_doctor_workinghours(doctor_id):
                cursor.execute(
                    '''SELECT doctor_workinghours FROM doctor WHERE doctor_id = %s''',
                    (doctor_id,))
                workinghours = cursor.fetchone()
                return workinghours[0] if workinghours else None

            doctor_workinghours = fetch_doctor_workinghours(doctor_id)

            if doctor_workinghours:
                rest_days = self.parse_doctor_workinghours(doctor_workinghours)
                if selected_date_obj.strftime('%A') in rest_days:
                    messagebox.showerror("Error",
                                         f"You are on a rest day on {selected_date}. No appointments can be made.")
                    return

            def fetch_appointments_for_doctor(selected_date):
                cursor.execute(
                    '''SELECT ar_time FROM appointment_request ar
                    JOIN appointment a ON ar.ar_id = a.ar_id
                    WHERE ar.ar_date = %s AND a.doctor_id = %s
                    AND ar.ar_status = 'ongoing' AND a.appointment_complete = 0''',
            ...
            def fetch_doctor_name(doctor_id):
                cursor.execute('''SELECT doctor_name FROM doctor WHERE doctor_id = %s''',
                               (doctor_id,))
                doctor_name = cursor.fetchone()
                return doctor_name[0] if doctor_name else None

            appointments = fetch_appointments_for_doctor(selected_date)
            # Convert database fetched times to datetime.time objects
            appointment_times = [datetime.strptime(str(app_time), '%H:%M:%S').time() for app_time in appointments]
            ...
            for widget in date_doctor_frame.winfo_children():
                if widget.winfo_y() > 100:  # Avoid removing the date_doctor_frame
                    widget.destroy()

            # Fetch and display the doctor's name
            doctor_name = fetch_doctor_name(doctor_id)
            self.doctor_name_label = tk.Label(date_doctor_frame, text=f"{doctor_name}",
                                              font=('Open Sans', 12, 'bold'), bg='white')
            self.doctor_name_label.place(x=500, y=120)

            columns = ("time", "status")
            self.tree = ttk.Treeview(date_doctor_frame, columns=columns, show='headings', height=14)
            self.tree.heading("time", text="Time")
            self.tree.heading("status", text="Appointment")
            self.tree.column("time", width=400, anchor=tk.CENTER)
            self.tree.column("status", width=400, anchor=tk.CENTER)

            # Define tag for booked appointments
            self.tree.tag_configure('Booked', foreground='red')

            for time_slot in time_slots:
                status = "Available" if time_slot not in appointment_times else "Booked"
                tag = 'Booked' if status == "Booked" else ''
                self.tree.insert("", tk.END, values=(time_slot.strftime('%H:%M:%S'), status), tags=(tag,))

            self.tree.place(x=150, y=150)

    def parse_doctor_workinghours(self, workinghours):
        # Parse the working hours to get rest days
        parts = workinghours.split(', except ')
        if len(parts) > 1:
            rest_days = parts[1].split(' and ')
        else:
            rest_days = []
        return rest_days
