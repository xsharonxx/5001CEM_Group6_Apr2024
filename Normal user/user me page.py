...
database = mysql.connector.connect(host="localhost", user="root", password="Jay_11022001", database="cad")
...
class User:
    def __init__(self, main_window, user_id):
        ...
        self.entries = []  # Initialize an empty list to store entry widgets
        self.eye_closed_image = load_image('eye closed', 24, 24)  # added for user show me
        self.eye_opened_image = load_image('eye opened', 24, 24)  # added for user show me
        ...
        # style config n style map for eye close button added
        style.configure('eye_closed_green.TButton', border=0, relief='flat', background='#D0F9EF',
                        image=self.eye_closed_image)
        style.map('eye_closed_green.TButton', background=[('active', '#D0F9EF')])
        # style config n style map for eye opened button added
        style.configure('eye_opened_green.TButton', border=0, relief='flat', background='#D0F9EF',
                        image=self.eye_opened_image)
        style.map('eye_opened_green.TButton', background=[('active', '#D0F9EF')])
        # style config n style map for select button added
        style.configure('selection.TButton', border=0, relief='flat', background='#D0F9EF', foreground='#3DAEC7',
                        font=('Rubik', 12, 'bold'))
        style.map('selection.TButton', background=[('active', '#D0F9EF')], foreground=[('active', '#0B8FAC')])
        self.me_frame = tk.Frame(self.window, width=1050, height=510, bg='white')

        # JUST EDITED-Initialize reset_password_frame here
        self.reset_password_frame = tk.Frame(self.window, width=1050, height=510, bg='white')
    ...
    def show_activity_frame(self, bar_width, bar_x, frame):
        ...
        # Call the set_up_me_frame method when the 'Me' frame is shown
        if frame == self.me_frame:
            self.set_up_me_frame()
    ...
    def get_user_info(self):
        ...
        cursor.execute('''
            SELECT u.user_email, u.user_password, p.patient_name, p.patient_ic_passport, p.patient_gender,
                   p.patient_contact, p.patient_address
            FROM user u
        ...
        if user_info:
            return {
                ...
                "Name": user_info[2],
                "IC or Passport Number": user_info[3],
                "Gender": user_info[4],
                "Contact Number": user_info[5],
                "Address": user_info[6]
            }
        else:
            return {}
    ...
    def set_up_me_frame(self):
        for widget in self.me_frame.winfo_children():
            widget.destroy()
        # THIS PART WAS ADDED
        self.me_frame.config(width=1050, height=510)
        ...
        # Email
        email_label = tk.Label(self.me_frame, text='Email:', font=('Open Sans', 12, 'bold'), bg='white', fg='#000000')
        email_label.place(x=50, y=50)
        labels.append(email_label)
        email_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=45, highlightbackground="#C8C7C7",
                                     highlightthickness=0.5)
        email_entry_frame.place(x=240, y=45)
        email_entry = tk.Entry(email_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585', border=0,
                               width=35)
        email_entry.place(x=10, y=12)
        email_entry.insert(0, user_info['Email'])
        self.entries.append(email_entry)# added
        # Disable editing and set color manually
        email_entry.config(state='disabled')
        email_entry.config(disabledforeground='#858585', disabledbackground='#D0F9EF')
        email_entry.bind('<FocusIn>', lambda event: self.focus_entry('entry', email_entry))
        email_entry.bind('<FocusOut>',
                           lambda event: self.leave_focus_entry('entry', email_entry, user_info['Email']))
        # Password
        password_label = tk.Label(self.me_frame, text="Password:", font=('Open Sans', 12, 'bold'), bg='white',
                                  fg='#000000')
        password_label.place(x=50, y=105)
        labels.append(password_label)
        password_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=45,
                                        highlightbackground="#C8C7C7",
                                        highlightthickness=0.5)
        password_entry_frame.place(x=240, y=100)
        password_entry = tk.Entry(password_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585', border=0,
                                  width=35, show='*')
        password_entry.place(x=10, y=12)
        password_entry.insert(0, user_info['Password'])
        self.entries.append(password_entry)  # added
        # Disable editing and set color manually
        password_entry.config(state='disabled')
        password_entry.config(disabledforeground='#858585', disabledbackground='#D0F9EF')

        password_eye_closed_button = ttk.Button(password_entry_frame, style='eye_closed_green.TButton', cursor='hand2')
        password_eye_closed_button.place(x=270, y=2)
        password_eye_opened_button = ttk.Button(password_entry_frame, style='eye_opened_green.TButton', cursor='hand2')
        password_visibility = tk.Label(password_entry_frame, text='Close')
        password_eye_closed_button.config(
            command=lambda: self.show_hide_password(password_entry, password_eye_opened_button,
                                                    password_eye_closed_button,
                                                    password_visibility))
        password_eye_opened_button.config(
            command=lambda: self.show_hide_password(password_entry, password_eye_opened_button,
                                                    password_eye_closed_button,
                                                    password_visibility))

        password_entry.bind('<FocusIn>',
                            lambda event: self.focus_entry('password', password_entry, password_visibility))
        password_entry.bind('<FocusOut>',
                            lambda event: self.leave_focus_entry('password', password_entry, user_info['Password']))


        # Name
        name_label = tk.Label(self.me_frame, text='Name:', font=('Open Sans', 12, 'bold'), bg='white', fg='#000000')
        name_label.place(x=50, y=165)
        labels.append(name_label)
        name_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=45, highlightbackground="#C8C7C7",
                                    highlightthickness=0.5)
        name_entry_frame.place(x=240, y=160)
        name_entry = tk.Entry(name_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585', border=0, width=35)
        name_entry.place(x=10, y=12)
        name_entry.insert(0, user_info['Name'])
        self.entries.append(name_entry)  # added
        # Disable editing and set color manually
        name_entry.config(state='disabled')
        name_entry.config(disabledforeground='#858585', disabledbackground='#D0F9EF')
        name_entry.bind('<FocusIn>', lambda event: self.focus_entry('entry', name_entry))
        name_entry.bind('<FocusOut>',
                         lambda event: self.leave_focus_entry('entry', name_entry, user_info['Name']))

        # IC or Passport Number
        ic_passport_label = tk.Label(self.me_frame, text='IC or Passport Number:', font=('Open Sans', 12, 'bold'),
                                     bg='white', fg='#000000')
        ic_passport_label.place(x=50, y=225)
        labels.append(ic_passport_label)
        ic_passport_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=45,
                                           highlightbackground="#C8C7C7",
                                           highlightthickness=0.5)
        ic_passport_entry_frame.place(x=240, y=220)
        ic_passport_entry = tk.Entry(ic_passport_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585',
                                     border=0, width=35)
        ic_passport_entry.place(x=10, y=12)
        ic_passport_entry.insert(0, user_info['IC or Passport Number'])
        self.entries.append(ic_passport_entry)  # added
        # Disable editing and set color manually
        ic_passport_entry.config(state='disabled')
        ic_passport_entry.config(disabledforeground='#858585', disabledbackground='#D0F9EF')
        ic_passport_entry.bind('<FocusIn>', lambda event: self.focus_entry('entry', ic_passport_entry))
        ic_passport_entry.bind('<FocusOut>',
                        lambda event: self.leave_focus_entry('entry', ic_passport_entry, user_info['IC or Passport Number']))
        # Gender
        gender_label = tk.Label(self.me_frame, text='Gender', font=('Open Sans', 12, 'bold'), bg='white', fg='#000000')
        gender_label.place(x=50, y=285)
        gender_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=45, highlightbackground="#C8C7C7",
                                      highlightthickness=0.5)
        gender_entry_frame.place(x=240, y=280)
        gender_entry = tk.Label(gender_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585')
        gender_entry.place(x=8, y=12)
        gender_entry.config(text=user_info['Gender'])
        self.gender_button = ttk.Button(gender_entry_frame, text='▼', style='selection.TButton', width=4,
                                        cursor='hand2',
                                        command=lambda: self.display_menu(gender_entry_frame, 1, 40, gender_menu),
                                        state='disabled')
        self.gender_button.place(x=265, y=5)
        gender_menu = tk.Menu(self.me_frame, tearoff=0, bg='#D0F9EF', fg='#333333', font=('Open Sans', 10))
        ...
        gender_menu.add_command(label="Clear", command=lambda: self.select_menu_option(gender_entry, 'Clear',
                                                                                       user_info['Gender']))
        gender_menu.add_command(label="Cancel\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t ",
                                command=gender_menu.unpost)
        # Contact Number
        contact_label = tk.Label(self.me_frame, text='Contact Number', font=('Open Sans', 12, 'bold'), bg='white',
                                    fg='#000000')
        contact_label.place(x=50, y=345)
        labels.append(contact_label)
        contact_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=45, highlightbackground="#C8C7C7",
                                          highlightthickness=0.5)
        contact_entry_frame.place(x=240, y=340)
        contact_entry = tk.Entry(contact_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585',
                                    border=0, width=35)
        contact_entry.place(x=10, y=12)
        contact_entry.insert(0, user_info['Contact Number'])
        self.entries.append(contact_entry)

        # Disable editing and set color manually
        contact_entry.config(state='disabled')
        contact_entry.config(disabledforeground='#858585', disabledbackground='#D0F9EF')
        contact_entry.bind('<FocusIn>', lambda event: self.focus_entry('entry', contact_entry))
        contact_entry.bind('<FocusOut>', lambda event: self.leave_focus_entry('entry', contact_entry,
                                                                              user_info['Contact Number']))

        # Address
        address_label = tk.Label(self.me_frame, text=f"Address: {user_info['Address']}", font=('Open Sans', 12,'bold'),
                                 bg='white', fg='#000000')
        address_label.place(x=50, y=405)
        labels.append(address_label)

        address_label = tk.Label(self.me_frame, text='Address', font=('Open Sans', 12, 'bold'), bg='white',
                                    fg='#000000')
        address_label.place(x=50, y=405)
        address_entry_frame = tk.Frame(self.me_frame, bg='#D0F9EF', width=320, height=85, highlightbackground="#C8C7C7",
                                          highlightthickness=0.5)
        address_entry_frame.place(x=240, y=400)
        address_entry = tk.Text(address_entry_frame, font=('Open Sans', 10), bg='#D0F9EF', fg='#858585', border=0,
                                   width=35,
                                   height=4, wrap='word')
        address_entry.place(x=8, y=12)
        address_entry.insert('1.0', user_info['Address'])
        self.entries.append(address_entry)  # added
        # Disable editing and set color manually
        address_entry.config(state='disabled')

        address_entry.bind('<FocusIn>', lambda event: self.focus_entry('text', address_entry))
        address_entry.bind('<FocusOut>',
                                        lambda event: self.leave_focus_entry('text', address_entry, user_info['Address']))


        # Reset Password Button
        reset_password_button = tk.Button(self.me_frame, text="Reset Password", command=lambda:self.show_reset_password_frame())
        reset_password_button.place(x=800, y=30)
        labels.append(reset_password_button)

        # Edit Button
        self.edit_button = tk.Button(self.me_frame, text='Edit', font=('Open Sans', 10, 'bold'), bg='#166E82',
                                     fg='white', command=lambda: self.enable_edit_mode)
        self.edit_button.place(x=900, y=30)
    ...
    def enable_edit_mode(self):
        for entry in self.entries:
            ...
            entry.config(fg='#858585', bg='#D0F9EF')  # Restore original colors

        self.gender_button.config(state='normal')  # Enable the gender button  # Enable the gender button
        self.edit_button.config(text='Save', command=self.save_user_info)
    ...
    def save_user_info(self):
        ...
        ic = self.entries[3].get()
        gender = self.entries[4].get()
        contact = self.entries[5].get()
        address = self.entries[6].get()

        cursor.execute('''
            UPDATE user
            SET user_email = %s, user_password = %s
            WHERE user_id = %s
        ''', (email, password, self.user_id))

        cursor.execute('''
            UPDATE patient
            SET patient_name = %s, patient_ic_passport = %s, patient_gender = %s, patient_contact = %s, patient_address = %s
            WHERE user_id = %s
        ''', (name, ic, gender, contact, address, self.user_id))

        database.commit()

        # Disable the entries after saving
        for entry in self.entries:
            entry.config(state='disabled')
            entry.config(disabledforeground='#858585', disabledbackground='#D0F9EF')
        self.edit_button.config(text='Edit', command=self.enable_edit_mode)
    def show_reset_password_frame(self):
        # EDITED NOW according to forget pass page

        def update_password():
            self.window.focus_set()
            print("reset_Pass")
            if old_password_entry.cget('fg') == '#333333' \
                    and new_password_entry.cget('fg') == '#333333' and confirm_password_entry.cget('fg') == '#333333':
                user_id = self.user_id
                cursor.execute('''SELECT user_password FROM user WHERE user_id=%s''', (user_id,))
                current_password = cursor.fetchone()
                if current_password:
                    if old_password_entry.get() == current_password[0]:
                        if len(new_password_entry.get()) >= 8:
                            if new_password_entry.get() == confirm_password_entry.get():
                                validate_update_label.config(text='')
                                new_password = new_password_entry.get()
                                cursor.execute('''UPDATE user SET user_password=%s WHERE user_id=%s''',
                                               (new_password, user_id))
                                database.commit()
                                messagebox.showinfo("Success", 'Password updated successfully')
                                self.show_activity_frame(60, 976, self.reset_password_frame)
                            else:
                                validate_update_label.config(text='Passwords do not match')
                        else:
                            validate_update_label.config(text='Password must be at least 8 characters')
                    else:
                        validate_update_label.config(text='Incorrect old password')
                else:
                    validate_update_label.config(text='User not found')
            else:
                validate_update_label.config(text='Please fill in all the details')

            # JUST EDITED-Ensure the frame is created and passed to the reset method
            self.reset(self.reset_password_frame)
            self.reset_password_frame.pack()
            self.me_frame.pack_forget()

            reset_password_background_label = tk.Label(self.reset_password_frame, image=self.lfr_background)
            reset_password_background_label.pack()

            title_label = tk.Label(self.reset_password_frame, text="Reset Password", font=('Open Sans', 30, 'bold'),
                                   bg='white',
                                   fg='#333333')
            title_label.place(x=620, y=35)

        # CORRECTED REFERENCE TO reset_password_frame INSTEAD OF frame
        old_password_label = tk.Label(self.reset_password_frame, text="Old Password", font=('Open Sans', 12, 'bold'), bg='white',
                                      fg='#000000')
        old_password_label.place(x=620, y=135)

        old_password_entry_frame = tk.Frame(self.reset_password_frame, bg='#F5F5F5', width=320, height=45,
                                            highlightbackground="#C8C7C7",
                                            highlightthickness=0.5)
        old_password_entry_frame.place(x=625, y=165)
        old_password_entry = tk.Entry(old_password_entry_frame, font=('Open Sans', 10), bg='#F5F5F5', fg='#858585',
                                      border=0, width=35)
        old_password_entry.place(x=10, y=12)
        old_password_entry.insert(0, 'Enter Old Password')
        old_password_entry.bind('<FocusIn>', lambda event: self.focus_entry('password', old_password_entry))
        old_password_entry.bind('<FocusOut>', lambda event: self.leave_focus_entry('password', old_password_entry,
                                                                                   'Enter Old Password'))
        old_password_entry.bind('<Return>', lambda event: update_password())

        new_password_label = tk.Label(self.reset_password_frame, text="New Password", font=('Open Sans', 12, 'bold'),
                                      bg='white',
                                      fg='#000000')
        new_password_label.place(x=620, y=230)
        new_password_entry_frame = tk.Frame(self.reset_password_frame, bg='#F5F5F5', width=320, height=45,
                                            highlightbackground="#C8C7C7",
                                            highlightthickness=0.5)
        new_password_entry_frame.place(x=625, y=260)
        new_password_entry = tk.Entry(new_password_entry_frame, font=('Open Sans', 10), bg='#F5F5F5', fg='#858585',
                                      border=0, width=35,
                                      show='')
        new_password_entry.place(x=10, y=12)
        new_password_entry.insert(0, 'Enter New Password')
        new_password_eye_closed_button = ttk.Button(new_password_entry_frame, style='eye_closed_grey.TButton',
                                                    cursor='hand2')
        new_password_eye_closed_button.place(x=270, y=2)
        new_password_eye_opened_button = ttk.Button(new_password_entry_frame, style='eye_opened_grey.TButton',
                                                    cursor='hand2')
        new_password_visibility = tk.Label(new_password_entry_frame, text='Close')
        new_password_eye_closed_button.config(
            command=lambda: self.show_hide_password(new_password_entry, new_password_eye_opened_button,
                                                    new_password_eye_closed_button,
                                                    new_password_visibility))
        new_password_eye_opened_button.config(
            command=lambda: self.show_hide_password(new_password_entry, new_password_eye_opened_button,
                                                    new_password_eye_closed_button,
                                                    new_password_visibility))
        new_password_entry.bind('<FocusIn>',
                                lambda event: self.focus_entry('password', new_password_entry, new_password_visibility))
        new_password_entry.bind('<FocusOut>', lambda event: self.leave_focus_entry('password', new_password_entry,
                                                                                   'Enter New Password'))
        new_password_entry.bind('<Return>', lambda event: update_password())

        confirm_password_label = tk.Label(self.reset_password_frame, text="Re-enter New Password",
                                          font=('Open Sans', 12, 'bold'),
                                          bg='white', fg='#000000')
        confirm_password_label.place(x=620, y=325)
        confirm_password_entry_frame = tk.Frame(self.reset_password_frame, bg='#F5F5F5', width=320, height=45,
                                                highlightbackground="#C8C7C7",
                                                highlightthickness=0.5)
        confirm_password_entry_frame.place(x=625, y=355)
        confirm_password_entry = tk.Entry(confirm_password_entry_frame, font=('Open Sans', 10), bg='#F5F5F5',
                                          fg='#858585', border=0, width=35,
                                          show='')
        confirm_password_entry.place(x=10, y=12)
        confirm_password_entry.insert(0, 'Re-enter New Password')
        confirm_password_eye_closed_button = ttk.Button(confirm_password_entry_frame, style='eye_closed_grey.TButton',
                                                        cursor='hand2')
        confirm_password_eye_closed_button.place(x=270, y=2)
        confirm_password_eye_opened_button = ttk.Button(confirm_password_entry_frame, style='eye_opened_grey.TButton',
                                                        cursor='hand2')
        confirm_password_visibility = tk.Label(confirm_password_entry_frame, text='Close')
        confirm_password_eye_closed_button.config(
            command=lambda: self.show_hide_password(confirm_password_entry, confirm_password_eye_opened_button,
                                                    confirm_password_eye_closed_button,
                                                    confirm_password_visibility))
        confirm_password_eye_opened_button.config(
            command=lambda: self.show_hide_password(confirm_password_entry, confirm_password_eye_opened_button,
                                                    confirm_password_eye_closed_button,
                                                    confirm_password_visibility))
        confirm_password_entry.bind('<FocusIn>', lambda event: self.focus_entry('password', confirm_password_entry,
                                                                                confirm_password_visibility))
        confirm_password_entry.bind('<FocusOut>',
                                    lambda event: self.leave_focus_entry('password', confirm_password_entry,
                                                                         'Re-enter New Password'))
        confirm_password_entry.bind('<Return>', lambda event: update_password())

        back_button = ttk.Button(self.reset_password_frame, text='Back', style='small_green.TButton', cursor='hand2',
                                 width=20,
                                 padding=5,
                                 command=lambda: self.set_up_me_frame())
        back_button.place(x=690, y=440)
        update_pass_button = ttk.Button(self.reset_password_frame, text='Update Password', style='small_green.TButton',
                                        cursor='hand2',
                                        width=20,
                                        padding=5, command=lambda: update_password())
        update_pass_button.place(x=690, y=490)
        validate_update_label = tk.Label(self.reset_password_frame, text='', font=('Open Sans', 8), bg='white',
                                         fg='red',
                                         anchor='center', width=32)
        validate_update_label.place(x=689, y=419)
    ...
    def show_edit_frame(self):
        pass  # Implement edit user information functionality here
class Clinic:
    def __init__(self, main_window, user_id):
        self.root_window = main_window
        self.user_id = user_id
