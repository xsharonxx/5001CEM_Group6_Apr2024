...
class User:
    ...
    def set_up_clinic_frame(self):
        def switch(frame, canvas, content):
            clinics_frame.pack_forget()
        ...
        def show_clinics():
            ...
            if search_entry.cget('fg') == '#858585':
                ...
                cursor.execute('''SELECT * FROM clinic WHERE clinic_status=%s''', (1, ))
            ...
            switch(clinics_frame, clinic_canvas, clinic_content_frame)
        ...
        def show_detail(c):
            ...
            cursor.execute('''SELECT * FROM doctor WHERE clinic_id=%s AND doctor_status=%s''', (clinic_id, 1))
            ...
            switch(detail_frame, detail_canvas, detail_content_frame)
            ...
            if schedule_doctor.cget('fg') == '#333333' and schedule_date.cget('fg') == '#333333' \
                ...
                if d is not None:
                    ...
                    cursor.execute('''INSERT INTO appointment_request (ar_date, ar_time, ar_detail, ar_status, 
                                                       ar_doctor, ar_ifreject, ar_datetime, patient_id, clinic_id) 
                                                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)''',
                                   (date, time, description, 'pending', 0, None, current, self.user_id, c[0]))
                    database.commit()
        ...
        def select_day(except_day, c, d=None):
            ...
            switch(schedule_frame, schedule_canvas, schedule_content_frame)
        ...
        def show_schedule(c):
            ...
            cursor.execute('''SELECT * FROM doctor WHERE clinic_id=%s AND doctor_status=%s''', (c[0], 1))
            doctors = cursor.fetchall()
            for doctor in doctors:
