...
class TestSetupClinicRequestFrame(unittest.TestCase):
    ...
    def test_show_clinic_request_success(self):
        # Mock the fetchall method to simulate database return values
        self.app.cursor.fetchall.return_value = [
            {'cr_id': 1, 'cr_type': 'join', 'cr_reason': 'Expanding services', 'cr_datetime': '2023-06-23', 'cr_detail':
                'Details here', 'clinic_name': 'New Clinic', 'clinic_contact': '1234567890', 'user_email': 'clinic@example.com', 'clinic_id': 1}
        ]

        self.app.set_up_clinic_request_frame()
        self.app.cursor.execute.assert_called_once()
        self.assertTrue(self.app.format_date.called)

        # Check if the clinic request frame contains the expected label
        labels = []
        for child in self.app.clinic_request_scrollable_frame.winfo_children():
            if isinstance(child, tk.Frame):
                for grandchild in child.winfo_children():
                    if isinstance(grandchild, tk.Label):
                        labels.append(grandchild.cget("text"))
        self.assertIn("Request ID: 1", labels, "Expected 'Request ID: 1' not found in the labels.")
